...
def firstMethod(X: Straight, Y: Straight):
    ...
    for i in range(len(Y) + 1):
        if i == 0:
            ...
            if Y_nums[i] == '1':
                printStep(' ', (-X).mult(len(Y)), f'-[X]_{X.suffix()} - корерктирующий шаг', is_result=True, is_plus=True)
                S = S + (-X).mult(len(Y))
                printStep(' ', S, f'S_{i}')
                S = S + X.equalize(target_len)
                printStep(' ', X.equalize(target_len), f'+[X]_{X.suffix()} * 2^-{len(X)} - корерктирующий шаг', is_result=True, is_plus=True)

            else:
                printStep(' ', '', f'корректирующие шаги не выполняем', is_result=True, is_plus=True, is_plus_crossed=True)
            printStep(' ', S, f'S_{i}')
        else:
            printStep(f'Y_-{i} = {Y_nums[i]}', S, f'S_{i}')
            if Y_nums[i] == '1':
...
def secondMethod(X: Straight, Y: Straight):
    ...
    print(f'Преобразуем разряды множителя [\u1EF8]_{Y.suffix()} = {Y.binary_repr}')
    S = Inverted('0', '0' * (target_len))
    for i in range(len(Y)):
        printStep(f'\u1EF9_-{i + 1} = {Y_nums[i]}', S, f'S_{i}')
        if Y_nums[i] == '1':
            printStep(' ', (-X).mult(len(Y) - i - 1).equalize(target_len), f'-[X]_{X.suffix()} * 2^-{i + 1}', is_result=True, is_plus=True)

            S = S + (-X).mult(len(Y) - i - 1)
        else:
            printStep(' ', (-X).mult(len(Y) - i - 1).equalize(target_len), f'-[X]_{X.suffix()} * 2^-{i + 1}', is_result=True, is_plus=True, is_plus_crossed=True)
